...
##### Simulation model in Section 3.4 with n0=120, p=80, q=20, r=7, h=10
...
# corx: [Sigma_x]^{|i-j|}=corx^{|i-j|}
# cory: [Sigma_epsilon]^{|i-j|}=Sigma_epsilon^{|i-j|}
# generating 200 replicates for a source data with nk=300, transferring level=10, rank_source=5
...
lamseq_w <-  c(0.01, 0.05, 0.1, 0.2, 0.3, 0.5, 0.6, 0.8, 0.9, 1, 1.2, 1.5, 2, 2.5, 3, 5, 7, 10, 12, 15,18,20,25, 30, 50)
...
Errorvec_transNR <- unlist(lapply(Errorvec_transNR, function(x){ sqrt( mean( (x$B[-1,]- targestset$B)^2 )) }))
