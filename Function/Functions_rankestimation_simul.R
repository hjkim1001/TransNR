generator_target_rankestimationsimul <- function(rep,n,p,q,r, corx=0.5,cory=0){
  ...
  B <- matrix(B1,p,q)
  u <- runif(p*q)
  B[which(u < 0.5)] <- B2[which(u<0.5)]
  svd_B <- svd(B)
  dthres <- svd_B$d
  dthres[-(1:r)] <- 0
  B <- svd_B$u %*% diag(dthres) %*% t(svd_B$v)
...
########### Fitting TransSCAD to multiple simulated example ##################
